...
FixSRD_MCS::FixSRD_MCS(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg),
  ...
  binsrd(NULL), stencil(NULL)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"lamda") == 0) {
      ...
      iarg += 3;
    } else if (strcmp(arg[iarg],"tstatpower") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix srd command");
      tstat_power = force->numeric(FLERR,arg[iarg+1]);
			tstat_power = MIN(0.05,tstat_power);
			tstat_power = MAX(0.3, tstat_power);
      iarg += 2;
    } else if (strcmp(arg[iarg],"tstat") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix srd command");
      if (strcmp(arg[iarg+1],"no") == 0) {
				tstat = 0;
				error->all(FLERR, "srd/mcs fix always  use tstat");
			}
...
void FixSRD_MCS::setup(int /*vflag*/)
{
  setup_bounds();
...
void FixSRD_MCS::post_force(int /*vflag*/)
{
  int i,m,ix,iy,iz;
...
void FixSRD_MCS::reset_velocities()
{
  ...
  if (tstat) {  // true
    for (i = 0; i < nbins; i++){
      ...
      // scale = scale factor for thermal velocity
			...
			drandom =  1+(tstat_power*random->uniform());
			irandom = static_cast<int> (2.0*random->uniform());
			sign = irandom % 2;
			scale = sign? drandom:1./drandom;
			drandom =  random->uniform();
//tbin = vbin[i].value[0]/(n-dof_tstat) * tfactor;
      tbin = vbin[i].value[0] * tfactor ;
			tbin = tbin * (scale*scale-1.) / temperature_srd;
			tbin = exp( -tbin) *pow(scale,dimension*(n-1));
			prob = MIN(1.0, tbin);
//      scalee= sqrt(temperature_srd/tbin);
      if ( prob > drandom) {
				vsq = 0.0;
				for (j = binhead[i]; j >= 0; j = binnext[j]) {
...
void FixSRD_MCS::vbin_comm(int ishift)
{
...
void FixSRD_MCS::vbin_pack(BinAve *vbin, int n, int *list, double *buf)
{
  int j;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRD_MCS::vbin_unpack(double *buf, BinAve *vbin, int n, int *list)
{
  int j;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRD_MCS::xbin_comm(int ishift, int nval)
{
  BinComm *bcomm1,*bcomm2;
  MPI_Request request1,request2;
...
void FixSRD_MCS::xbin_pack(BinAve *vbin, int n, int *list, double *buf, int nval)
{
  int j, k;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRD_MCS::xbin_unpack(double *buf, BinAve *vbin, int n, int *list, int nval)
{
  int j, k;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void FixSRD_MCS::collisions_single()
{
...
void FixSRD_MCS::collisions_multi()
{
...
int FixSRD_MCS::inside_sphere(double *xs, double *xb, Big *big)
{
  double dx,dy,dz;
...
int FixSRD_MCS::inside_ellipsoid(double *xs, double *xb, Big *big)
{
  double x,y,z;
...
int FixSRD_MCS::inside_line(double *xs, double *xb, double *vs, double *vb,
...
int FixSRD_MCS::inside_tri(double *xs, double *xb, double *vs, double *vb,
                       Big *big, double dt_step)
{
...
int FixSRD_MCS::inside_wall(double *xs, int iwall)
{
...
double FixSRD_MCS::collision_sphere_exact(double *xs, double *xb,
...
void FixSRD_MCS::collision_sphere_inexact(double *xs, double *xb,
                                      Big *big,
                                      double *xscoll, double *xbcoll,
                                      double *norm)
{
...
double FixSRD_MCS::collision_ellipsoid_exact(double *xs, double *xb,
                                         double *vs, double *vb, Big *big,
                                         double *xscoll, double *xbcoll,
                                         double *norm)
{
...
void FixSRD_MCS::collision_ellipsoid_inexact(double *xs, double *xb,
                                         Big *big,
                                         double *xscoll, double *xbcoll,
                                         double *norm)
{
...
double FixSRD_MCS::collision_line_exact(double * /*xs*/, double * /*xb*/,
                                    double * /*vs*/, double * /*vb*/, Big * /*big*/,
                                    double dt_step,
                                    double *xscoll, double *xbcoll,
                                    double *norm)
...
double FixSRD_MCS::collision_tri_exact(double * /*xs*/, double * /*xb*/,
                                   double * /*vs*/, double * /*vb*/, Big * /*big*/,
                                   double dt_step,
                                   double *xscoll, double *xbcoll,
                                   double *norm)
...
double FixSRD_MCS::collision_wall_exact(double *xs, int iwall, double *vs,
                                     double *xscoll, double *xbcoll,
                                     double *norm)
{
  int dim = wallwhich[iwall] / 2;
...
void FixSRD_MCS::collision_wall_inexact(double *xs, int iwall, double *xscoll,
...
void FixSRD_MCS::slip(double *vs, double *vb, double *xb, Big *big,
...
void FixSRD_MCS::slip_wall(double *vs, int iwall, double *norm, double *vsnew)
{
...
void FixSRD_MCS::noslip(double *vs, double *vb, double *xb, Big *big, int iwall,
                    double *xsurf, double *norm, double *vsnew)
{
  double vs_dot_n,scale,r1,r2,vnmag,vtmag1,vtmag2;
  double tangent1[3],tangent2[3];
...
void FixSRD_MCS::force_torque(double *vsold, double *vsnew,
                          double *xs, double *xb,
                          double *fb, double *tb)
{
...
void FixSRD_MCS::force_wall(double *vsold, double *vsnew, int iwall)
...
int FixSRD_MCS::update_srd(int i, double dt, double *xscoll, double *vsnew,
                       double *xs, double *vs)
{
  int ix,iy,iz;
...
void FixSRD_MCS::setup_velocity_bins()
{
...
void FixSRD_MCS::setup_velocity_shift(int ishift, int dynamic)
{
...
void FixSRD_MCS::setup_search_bins()
{
...
void FixSRD_MCS::setup_search_stencil()
{
...
double FixSRD_MCS::point_bin_distance(double *x, int i, int j, int k)
{
...
double FixSRD_MCS::bin_bin_distance(int i, int j, int k)
{
  double delx,dely,delz;
...
int FixSRD_MCS::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixSRD_MCS::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
double FixSRD_MCS::compute_vector(int n)
{
...
void FixSRD_MCS::velocity_stats(int groupnum)
{
...
double FixSRD_MCS::newton_raphson(double t1, double t2)
{
...
void FixSRD_MCS::lineside(double t, double &f, double &df)
{
  double p[2],c[2];
...
void FixSRD_MCS::triside(double t, double &f, double &df)
{
  double p[2],c[2];
...
double FixSRD_MCS::memory_usage()
{
  double bytes = 0.0;
...
double FixSRD_MCS::distance(int i, int j)
{
...
void FixSRD_MCS::print_collision(int i, int j, int ibounce,
