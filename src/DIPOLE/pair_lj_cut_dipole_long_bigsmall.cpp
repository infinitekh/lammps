...
#include "pair_lj_cut_dipole_long_bigsmall.h"
#include <mpi.h>
#include <cmath>
#include <cstring>
#include "atom.h"
...
PairLJCutDipoleLongBigsmall::PairLJCutDipoleLongBigsmall(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
...
PairLJCutDipoleLongBigsmall::~PairLJCutDipoleLongBigsmall()
{
  if (allocated) {
...
void PairLJCutDipoleLongBigsmall::compute(int eflag, int vflag)
{
...
//  double *q = atom->q;
  ...
  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
		if (mask[i] & ibiggroupbit)  continue;
...
//    qtmp = atom->q[i];
    ...
    for (jj = 0; jj < jnum; jj++) {
      j = jlist[jj];
			if (mask[j] & ibiggroupbit)  continue;
...
//          g1 = qtmp*pjdotr - q[j]*pidotr + pdotp;
          ...
          if (factor_coul > 0.0) {
            ...
            zdix = delx * b2*pjdotr - b1*mu[j][0];
            zdiy = dely * b2*pjdotr - b1*mu[j][1];
            zdiz = delz * b2*pjdotr - b1*mu[j][2];
            zdjx = delx * b2*pidotr - b1*mu[i][0];
            zdjy = dely * b2*pidotr - b1*mu[i][1];
            zdjz = delz * b2*pidotr - b1*mu[i][2];
 ...
 */
            ...
            zaix = delx * d2*pjdotr - d1*mu[j][0];
            zaiy = dely * d2*pjdotr - d1*mu[j][1];
            zaiz = delz * d2*pjdotr - d1*mu[j][2];
            zajx = delx * d2*pidotr - d1*mu[i][0];
            zajy = dely * d2*pidotr - d1*mu[i][1];
            zajz = delz * d2*pidotr - d1*mu[i][2];
...
void PairLJCutDipoleLongBigsmall::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJCutDipoleLongBigsmall::settings(int narg, char **arg)
{
  if (narg < 1 || narg > 2)
...
void PairLJCutDipoleLongBigsmall::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 5)
...
double PairLJCutDipoleLongBigsmall::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCutDipoleLongBigsmall::init_style()
{
...
void PairLJCutDipoleLongBigsmall::write_restart(FILE *fp)
{
...
void PairLJCutDipoleLongBigsmall::read_restart(FILE *fp)
{
...
void PairLJCutDipoleLongBigsmall::write_restart_settings(FILE *fp)
{
...
void PairLJCutDipoleLongBigsmall::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void *PairLJCutDipoleLongBigsmall::extract(const char *str, int &dim)
{
